...
const BookApointment = () => {
  ...
  useEffect(() => {
    if (selectedDate) {
      const fetchAvailableSlots = async () => {
        try {
          const response = await axios.get(
            `${import.meta.env.VITE_BACKEND_URI}/api/reception/available-slots?date=${encodeURIComponent(
              selectedDate
            )}`,{withCredentials:true}
          );
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setError(null); // Reset any previous error
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URI}/api/reception/book-appointment`,
        data,{withCredentials:true}
      );
      setResponseData(response.data);
      toast.success(
  ...
  return (
    <div className='max-w-4xl my-10 bg-gradient-to-b from-blue-400 via-blue-500 to-cyan-500 mx-auto p-4 rounded-lg shadow-xl'>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
        {/* Full Name, Contact Number, and Age */}
        <div className='flex flex-col gap-6 md:flex-row'>
          <div className='flex-1'>
            <label
              htmlFor='fullName'
