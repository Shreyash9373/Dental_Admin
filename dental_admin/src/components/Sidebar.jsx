// ANIKET
import React from "react";
import { NavLink } from "react-router-dom";
import logo from "../assets/Dr.Pakhare1.jpeg";
import { useAuth } from "../context/AuthContext";
import { IoClose } from "react-icons/io5";

const Sidebar = ({ isHamburgerOpen, setIsHamburgerOpen }) => {
  const { authUser } = useAuth();

  return (
    <>
      {/* Overlay */}
      {isHamburgerOpen && (
        <div
          className="h-screen w-screen absolute z-40 top-0 bg-black opacity-50"
          onClick={() => setIsHamburgerOpen(false)} // Close sidebar when clicking outside
        ></div>
      )}

      {/* Sidebar */}
      <div
        className={`transition-transform duration-200 ease-in-out sticky z-50 top-0 left-0 min-h-screen bg-[#062335] text-white w-64 p-4 flex flex-col justify-between ${
          isHamburgerOpen ? "translate-x-0" : "-translate-x-64"
        } lg:translate-x-0`}
      >
        {/* Top Section */}
        <div>
          {/* Logo */}
          <div className="flex justify-center items-center gap-5">
            <img
              src={logo}
              alt="Logo"
              className="h-32 w-32 rounded-full mb-5 shadow-gray-600 shadow-lg"
            />
            <button
              onClick={() => setIsHamburgerOpen((prev) => !prev)}
              className="text-white text-4xl self-start lg:hidden"
            >
              <IoClose />
            </button>
          </div>

          {/* Navigation Links */}
          <nav className="border-t border-gray-600 pt-4">
            <ul className="space-y-4">
              {/* Reception Role */}
              {authUser?.role === "reception" && (
                <>
                  <li>
                    <NavLink
                      onClick={() => setIsHamburgerOpen(false)}
                      to="/admin/dashboard/user-enquiry"
                      className={({ isActive }) =>
                        `${isActive ? "text-blue-400 font-bold" : "hover:text-gray-300 font-normal"} w-full inline-block`
                      }
                    >
                      User's Enquiry
                    </NavLink>
                  </li>
                  <li>
                    <NavLink
                      onClick={() => setIsHamburgerOpen(false)}
                      to="/admin/dashboard/schedule-appointments"
                      className={({ isActive }) =>
                        `${isActive ? "text-blue-400 font-bold" : "hover:text-gray-300 font-normal"} w-full inline-block`
                      }
                    >
                      Scheduled Appointments
                    </NavLink>
                  </li>
                  <li>
                    <NavLink
                      onClick={() => setIsHamburgerOpen(false)}
                      to="/admin/dashboard/book-apointment"
                      className={({ isActive }) =>
                        `${isActive ? "text-blue-400 font-bold" : "hover:text-gray-300 font-normal"} w-full inline-block`
                      }
                    >
                      Book Appointment
                    </NavLink>
                  </li>
                </>
              )}

              {/* Doctor Role */}
              {authUser?.role === "doctor" && (
                <>
                  <li className="pt-8 border-t border-gray-700">
                    <NavLink
                      onClick={() => setIsHamburgerOpen(false)}
                      to="/admin/dashboard/see-appointment"
                      className={({ isActive }) =>
                        `${isActive ? "text-blue-400 font-bold" : "hover:text-gray-300 font-normal"} w-full inline-block`
                      }
                    >
                      See Appointments
                    </NavLink>
                  </li>
                  <li>
                    <NavLink
                      onClick={() => setIsHamburgerOpen(false)}
                      to="/admin/dashboard/add-event"
                      className={({ isActive }) =>
                        `${isActive ? "text-blue-400 font-bold" : "hover:text-gray-300 font-normal"} w-full inline-block`
                      }
                    >
                      Add Events
                    </NavLink>
                  </li>
                  <li>
                    <NavLink
                      onClick={() => setIsHamburgerOpen(false)}
                      to="/admin/dashboard/blogs"
                      className={({ isActive }) =>
                        `${isActive ? "text-blue-400 font-bold" : "hover:text-gray-300 font-normal"} w-full inline-block`
                      }
                    >
                      Add Blogs
                    </NavLink>
                  </li>
                  <li>
                    <NavLink
                      onClick={() => setIsHamburgerOpen(false)}
                      to="/admin/dashboard/updatepassword"
                      className={({ isActive }) =>
                        `${isActive ? "text-blue-400 font-bold" : "hover:text-gray-300 font-normal"} w-full inline-block`
                      }
                    >
                      Update Password
                    </NavLink>
                  </li>
                </>
              )}
            </ul>
          </nav>
        </div>
      </div>
    </>
  );
};

export default Sidebar;
