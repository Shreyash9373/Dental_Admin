...
const LoginPage = () => {
  const [currentuser, setCurrentUser] = useState("receptionist");
  ...
  const handleUserChange = () => {
    setCurrentUser((prev) => (prev === "doctor" ? "receptionist" : "doctor"));
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const response = await axios.post("http://localhost:4000/api/dashboard/login", data);
      const { name, role, token } = response.data;
      setResponseData(response.data)
      // Update the global user state
      setUser({ name, role, status: true });
    ...
    <div className='w-screen h-screen flex justify-center items-center'>
      <div className='shadow-2xl bg-gradient-to-b from-blue-400 via-blue-500 to-cyan-500 shadow-gray-500 p-12 flex flex-col gap-2'>
        <h1 className='text-white font-semibold mb-7 text-center text-xl md:text-3xl lg:text-5xl'>
          <span className='capitalize'>{user?.name || currentuser}</span> Login
        </h1>
        {/* Login form */}
        <form
        ...
        <div className='text-sm text-center'>
          ...
          <button
            ...
            <span className='capitalize'>
              {user?.role === "doctor" ? "receptionist" : "doctor"}
            </span>
          </button>
        </div>
      </div>
